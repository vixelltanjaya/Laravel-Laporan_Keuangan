...
<div class="row">
    <div class="col-lg-12">
        ...
        <a href="{{ url()->previous() }}" class="btn btn-default"> <i class="fas fa-angle-left me-1"></i>Back</a>
                ...
                <form id="masterJournalForm" action="{{ route('master-journal.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="evidence_id" id="evidence_id" class="form-control" required>
                            ...
                            <option value="{{ $evidence->id }}">{{ $evidence->prefix_code }} - {{ $evidence->code_title }}</option>
                            @endforeach
                        </select>
                    </div>
    ...
    $(document).ready(function() {
        ...
        $('button[data-target="#previewModal"]').on('click', function() {
            ...
            $('.journal-entries .form-group.row').each(function() {
                ...
                console.log("Apa nama noAccount" +noAccount);
                console.log("Apa nama account sign" +accountSign);
