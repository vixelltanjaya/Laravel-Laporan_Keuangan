...
class PelunasanPariwisataController extends Controller
{
    public function index($id)
    {
        ...
        return view('user-accounting.pelunasan-pariwisata', [
    ...
    {
        Log::debug('masuk func store' . json_encode($request->all()));
        ...
        try {
            ...
            foreach ($refs as $ref) {
                ...
                $editJournalEntry->is_reversed = false;
                $editJournalEntry->reversed_by = '';
                // $editJournalEntry->reversed_at = '';
            ...
            return redirect()->route('pelunasan-pariwisata.index', ['id' => $id])->withErrors($e->errors())->withInput();
        } catch (Exception $e) {
            DB::rollback();
            Log::error('Error storing cash out transaction: ' . $e->getMessage());
            return redirect()->route('pelunasan-pariwisata.index', ['id' => $id])->with('gagal', 'Transaksi gagal. ' . $e->getMessage());
        }
    }
}
